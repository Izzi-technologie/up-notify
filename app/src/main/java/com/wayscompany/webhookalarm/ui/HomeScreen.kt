...
import com.wayscompany.webhookalarm.ui.components.rememberPhoneAlertReadiness
...
fun HomeScreen(
    ...
    val readiness = rememberPhoneAlertReadiness()
    val showPermissions = !context.isTelevisionDevice() && !readiness.allGranted
    ScreenShell {
        if (showPermissions) {
            item {
                PhoneAlertPermissionsCard(readiness = readiness)
            }
        }
        item {
            HomeOverview(
